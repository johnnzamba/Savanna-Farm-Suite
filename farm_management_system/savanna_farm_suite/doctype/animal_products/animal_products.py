...
class AnimalProducts(Document):
    ...
    def on_update(self):
        ...
        if getattr(self, "product_selling_price", None) is None:
            return
        ...
        try:
            ...
            try:
                new_price = float(self.product_selling_price)
            except (TypeError, ValueError):
                frappe.log_error(f"Invalid selling price: {self.product_selling_price}", "CropProducts.on_update")
                return
            ...
            if existing_price:
                ...
                item_price = frappe.get_doc({
                    "doctype": "Item Price",
                    "item_code": self.name,
                    "uom": self.default_unit_of_measure or frappe.db.get_value("Item", self.name, "stock_uom"),
                    "price_list": "Standard Selling",
                    "selling": 1,
                    "currency": "KES",
